        ...
        <div class="demo-ba-grid reveal reveal-scale">
          ...
          <div class="demo-ba-col is-before">
            ...
            <div class="demo-window">
              ...
              <div class="demo-video-wrap">
                <video id="pk-video-before" src="https://previewkit-pitch.vercel.app/assets/Previewkit-Before1.mp4"
          ...
          <div class="demo-ba-col is-after">
            ...
            <div class="demo-window">
              ...
              <div class="demo-video-wrap">
                <video id="pk-video-after" src="https://previewkit-pitch.vercel.app/assets/Previewkit-After1.mp4"
                  muted playsinline loop preload="metadata"></video>
...
<span style="color:rgba(232,201,207,.45)">&lt;!-- Paste into your product template --&gt;</span>

&lt;div id="pk-widget"&gt;&lt;/div&gt;

&lt;script src="https://previewkit-demo.onrender.com/sdk/previewkit.js?v=2.7.0"&gt;&lt;/script&gt;
...
&lt;/script&gt;
          ...
          <a href="#contact" class="demo-shopify-cta">Get your snippet ${icon('arrow')}</a>
        </div>
  ...
  const PLANS = [
    {
      ...
      popular: false,
      features: ['Widget embed', '2 product templates', 'Email delivery to seller'],
    },
    {
      id: 'starter',
      name: 'Starter',
    ...
    },
    {
      ...
      cta: 'Start 14-day trial',
      popular: true,
      features: ['Multi-product templates', 'Webhook support', 'Seller dashboard', 'Priority support'],
    },
    {
      id: 'scale',
      name: 'Scale',
  ...
  function pricingCheckIcon() {
    return `<svg class="pricing-check-icon" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="8" cy="8" r="8" fill="rgba(232,201,207,0.15)"/>
      <path d="M5 8l2 2 4-4" stroke="#E8C9CF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`;
  }

  function renderPricingCards() {
    return PLANS.map(plan => {
      const price = pricingAnnual ? plan.annualPrice : plan.monthlyPrice;
      return `
        <div class="pricing-card${plan.popular ? ' pricing-card--popular' : ''} reveal reveal-delay-${PLANS.indexOf(plan) + 1}">
          ${plan.popular ? '<div class="pricing-popular-badge">Most Popular</div>' : ''}
          <div class="pricing-card-name">${plan.name}</div>
          <div class="pricing-card-price">
            <span class="pricing-price-amount">$${price}</span>
            ${price > 0 ? '<span class="pricing-price-mo">/mo</span>' : ''}
          </div>
          <div class="pricing-usage-pill">
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
              <circle cx="6" cy="6" r="5" stroke="currentColor" stroke-width="1"/>
              <path d="M6 3.5v3l1.5 1.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/>
            </svg>
            ${plan.usage}
          </div>
          <p class="pricing-card-desc">${plan.description}</p>
          <ul class="pricing-features">
            ${plan.features.map(f => `<li>${pricingCheckIcon()} ${f}</li>`).join('')}
          </ul>
          <a href="#contact" class="pricing-cta${plan.popular ? ' pricing-cta--primary' : ''}">${plan.cta}</a>
        </div>`;
    }).join('');
  }
  ...
  function renderPricing() {
      ...
      <div class="container">
        <div class="section-inner">
          ...
          <div class="pricing-header reveal">
            <div class="eyebrow pricing-eyebrow">Pricing</div>
            <h2 class="section-headline pricing-headline">
              Pay for what you sell,<br>not what you preview.
            </h2>
            <p class="section-sub pricing-sub">
              Simple plans based on confirmed customized orders per month.<br>
              No transaction fees. No surprises.
            </p>
          </div>

          <div class="pricing-toggle-wrap reveal">
            <div class="pricing-toggle" id="pk-pricing-toggle">
              <button class="pricing-toggle-btn active" data-period="monthly" id="pk-toggle-monthly">
                Monthly
              </button>
          ...
          <div class="pricing-cards" id="pk-pricing-cards">
            ${renderPricingCards()}
          </div>

          <div class="pricing-overage reveal">
            <div class="pricing-overage-left">
              <p class="pricing-overage-title">Need more orders?</p>
              <p class="pricing-overage-sub">Overage kicks in automatically — no plan changes, no interruptions. All paid plans.</p>
            </div>
            <div class="pricing-overage-pill">
              <span class="pricing-overage-amount">+$0.05</span>
              <span class="pricing-overage-label"> per order above limit</span>
            </div>
          </div>
    ...
    /* Toggle logic */
    const toggleEl = document.getElementById('pk-pricing-toggle');
    if (!toggleEl) return;

    toggleEl.addEventListener('click', function (e) {
      const btn = e.target.closest('.pricing-toggle-btn');
      if (!btn) return;

      const period = btn.dataset.period;
      pricingAnnual = period === 'annual';

      toggleEl.querySelectorAll('.pricing-toggle-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');

      const cardsEl = document.getElementById('pk-pricing-cards');
      if (cardsEl) {
        cardsEl.innerHTML = renderPricingCards();
        initScrollReveal();
      }
    });
  }
  ...
  function initCarousel() {
    ...
    if (prevControl) prevControl.addEventListener('click', () => { goTo(current - 1); startAuto(); });
    if (nextControl) nextControl.addEventListener('click', () => { goTo(current + 1); startAuto(); });
